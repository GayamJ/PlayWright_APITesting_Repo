...
//Assertions to verify the delete operation: this is actual test.
expect(deletebookingresponse.status()).toBe(201);
expect(deletebookingresponse.statusText()).toBe('Created');

console.log('DELETE booking response status for booking ID:', newbookingid, deletebookingresponse.status());
console.log('DELETE booking response status text:', deletebookingresponse.statusText());

//Test5: Try to get the deleted booking to confirm it's deleted

const getdeletedbookingresponse = 
await request.get(`https://restful-booker.herokuapp.com/booking/${newbookingid}`);
expect(getdeletedbookingresponse.status()).toBe(404);
console.log(`GET deleted booking response status for booking ID: ${newbookingid}, status: ${getdeletedbookingresponse.status()}`);

console.log('GET deleted booking response status text:',  getdeletedbookingresponse.statusText());
