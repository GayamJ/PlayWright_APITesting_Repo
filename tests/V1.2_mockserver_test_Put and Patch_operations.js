...
// Start the CommonJS mock server dynamically in a beforeAll hook so ESM tests
// don't use `require` and we can pick an ephemeral port.
process.env.PORT = process.env.PORT || '0';
let server;
let BASE;

// Run tests in this file serially so the mock server's in-memory state
// (the `post` object) is preserved between the PUT and subsequent GET.
test.describe.serial('Mock server PUT/PATCH operations', () => {
    test.beforeAll(async () => {
        const mod = await import('../mock-server');
        // When importing a CommonJS module via dynamic import, the export is on `default`.
...
test('Test3: Get Request after Put Request test', async ({request}) => { 
    ...
    // Close the mock server after all tests in this describe
    test.afterAll(async () => {
